...
export function OverviewView({ derived, trades, payouts, settings }) {
  ...
  const activeAccounts = derived.accounts.filter((a) => !a.archived && ["phase_1", "phase_2", "funded"].includes(a.status)).length;
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 mb-6">
        <div className="lg:col-span-3">
          {curve.length > 1 ? (
            <EquityCurve data={curve} height={260} gradientId="pdAreaOverview" title="Cumulative P&L" size={derived.accounts.filter((a) => !a.archived).reduce((s, a) => s + a.size, 0)} />
          ) : (
